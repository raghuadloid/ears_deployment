    ...
    with mp_face_mesh.FaceMesh(static_image_mode=True, max_num_faces=1, refine_landmarks=True, min_detection_confidence=0.5) as face_mesh:
        ...
        if results.multi_face_landmarks:
            ...
            if results.multi_face_landmarks:
                ...
                dist_eye_nose = abs(results.multi_face_landmarks[0].landmark[234].x *
                                    im_shape[1] - results.multi_face_landmarks[0].landmark[454].x * im_shape[1])/6

                top_left_x = int(
                    (results.multi_face_landmarks[0].landmark[234].x)*im_shape[1] - dist_eye_nose)
                top_left_y = int(
                    (results.multi_face_landmarks[0].landmark[10].y)*im_shape[0])

                bottom_right_x = int(
                    (results.multi_face_landmarks[0].landmark[454].x) * im_shape[1] + dist_eye_nose)
                bottom_right_y = int(
                    (results.multi_face_landmarks[0].landmark[152].y) * im_shape[0])
...
@app.post("/showcroprotate2")
async def show_image(file: UploadFile = File(...)):
    image_orig = read_file_as_image(await file.read())
    image = rotateandcrop2(image_orig)
    image = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)
    cv2.imwrite("out.png", image)
    return FileResponse("out.png")
